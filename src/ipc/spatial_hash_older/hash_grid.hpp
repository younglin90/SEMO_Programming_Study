...
#include <ipc/spatial_hash_older/collision_candidate.hpp>
...
public:
    ...
    inline const Eigen::Vector3d& getMin() const { return min; }
    inline const Eigen::Vector3d& getMax() const { return max; }
    inline const Eigen::Vector3d& getHalfExtent() const { return half_extent; }
...
class HashGrid {
public:
    void resize(Eigen::Vector3d min, Eigen::Vector3d max, double cellSize);
    ...
    void addEdge(
        const Eigen::Vector3d& edge_vertex0_t0,
        const Eigen::Vector3d& edge_vertex1_t0,
        const Eigen::Vector3d& edge_vertex0_t1,
        const Eigen::Vector3d& edge_vertex1_t1,
        const long index,
        const double inflation_radius = 0.0);
