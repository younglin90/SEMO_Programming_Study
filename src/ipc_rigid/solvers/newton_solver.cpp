...
#include "../utils_custom/utils_custom.hpp"
...
OptimizationResults NewtonSolver::solve(const Eigen::VectorXd& x0)
{
    ...
    for (iteration_number = 0; iteration_number < max_iterations;
        ...
        slice(gradient, free_dof, gradient_free);
        slice(hessian, free_dof, free_dof, hessian_free);
        ...
        slice_into(gradient_free, free_dof, grad_direction);
        slice_into(direction_free, free_dof, direction);
           ...
           //     sqrt(abs(grad_fx.dot(dir))), lower_bound, fxi - fx,
            ...
            sample_search_direction(
                ...
                [&](const Eigen::VectorXd& x, Eigen::VectorXd& grad) {
                    ...
                    slice(grad, free_dof, grad_free);
                    grad.setZero();
                    slice_into(grad_free, free_dof, grad);
                    return fx;
                },
                max_step_size);
        }
